...
// Global declarations for Teachable Machine library (loaded via CDN)
...
class Config {
    ...
    static readonly MODEL_URL = "https://teachablemachine.withgoogle.com/models/bjmc_9S6A/";

    // Konfigurasi deteksi
    static readonly MIN_CONFIDENCE = 0.7; // Minimum 70% confidence untuk dianggap valid
    static readonly PREDICTION_INTERVAL = 100; // Update setiap 100ms (10 FPS)
    static readonly MAX_HISTORY = 10; // Maksimal item di riwayat
}
...
class AppState {
    ...
    uptimeInterval: any = null;
    facingMode: 'user' | 'environment' = 'user'; // Default camera (front)
}
...
class DOMElements {
    ...
    realtimeBadge: HTMLElement;

    // Stats
    totalDetected: HTMLElement;
    avgAccuracy: HTMLElement;
    uptime: HTMLElement;
    streamQuality: HTMLElement;
    inferenceTime: HTMLElement;
    ...
    constructor() {
        ...
        this.btnStart = this.getElement('btn-start') as HTMLButtonElement;
        this.btnStop = this.getElement('btn-stop') as HTMLButtonElement;
        this.btnSwitch = this.getElement('btn-switch-camera') as HTMLButtonElement;
    ...
    private getElement(id: string): any {
...
class ToolDetectionApp {
    ...
    private async predict(): Promise<void> {
        try {
            ...
            const predictions: Prediction[] = await this.state.model.predict(this.state.webcam.canvas);
     ...
     */
    private updateUI(prediction: Prediction): void {
     ...
     * Update statistics
     */
    private updateStats(prediction: Prediction): void {
     ...
     */
    private addToHistory(detection: DetectionHistory): void {
...
(window as any).app = ToolDetectionApp;
